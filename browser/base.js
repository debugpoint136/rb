...
function toggle1(event){
    ...
    var d=make_controlpanel(cp_publichub);
	...
	dom_create('div',d,'display:inline-block;margin-right:20px;',{c:'whitebar',
		...
		clc:toggle7_2});
	if(gflag.browser.genome.publichub.lst.length>0) {
...
// show hmtk facet panel
	if(apps.hmtk.main.style.display=="none") {
		cloakPage();
		var b=gflag.browser.facet.main;
		b.style.display='block';
		stripChild(apps.hmtk.main.__contentdiv,0);
		apps.hmtk.main.__contentdiv.appendChild(b);
		panelFadein(apps.hmtk.main, 100+document.body.scrollLeft, 50+document.body.scrollTop);
		var b=gflag.browser;
		apps.hmtk.bbj=b;
		// dataset and publichub panel belong to the genome, add it to facet panel holder
		b.facet.content_dataset.appendChild(b.genome.datasetHolder);
		b.facet.content_hub.appendChild(b.genome.publichub.holder);
	} else {
...
// Browser.prototype.toggle1=function() // dpuru : Jun 24 testing Tracks feature
function toggle1_test(event)
{
// show hmtk facet panel
    if(apps.hmtk.main.style.display=="none") {
        cloakPage();
        var b=gflag.browser;
        if(b.trunk) b=b.trunk;
        /* var b=this;
        if(this.trunk) b=this.trunk;*/
        // start including facet main code
        b.facet={
        ...
        var d=dom_create('div',b.facet.main,'display:table;margin:0px 20px 20px 0px;');
        // actual holder
        ...
        b.facet.dim2.dom=s;
        s.onclick=facet_dimension_show;
        b.facet.swapbutt=dom_create('div',d3,'display:none;margin-left:20px;',{t:'&#8646;',c:'mdt_box',clc:function(){bbj.facet_swap();},title:'swap row/column'});
...
for(var i=0; i<json.length; i++) {
	...
	if(hubtagistrack(tag)) {
		...
		//dpuru // alertbox_addmsg({text:'Use of obsolete attribute "native_metadata_terms" in datahub (see '+FT2noteurl.md+')'});
